...
public class ExchangeRateController {
    ...
    @Operation(summary = "특정 날짜 환율 조회", description = "특정 날짜의 모든 환율 정보를 조회합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공")
    })
    ...
    public ResponseEntity<ApiResponse<List<ExchangeRateResponse>>> getRatesByDate(
        ...
        List<ExchangeRate> rates = exchangeRatePort.getRatesByDate(date);
        List<ExchangeRateResponse> response = rates.stream()
                .map(ExchangeRateResponse::from)
                .toList();
        return ResponseEntity.ok(ApiResponse.success("환율 히스토리를 조회했습니다.", response));
    }

    @Operation(summary = "환율 추이 조회", description = "특정 통화 쌍의 기간별 환율 추이를 조회합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공")
    })
    @GetMapping("/history")
    public ResponseEntity<ApiResponse<List<ExchangeRateResponse>>> getHistory(
        ...
        List<ExchangeRate> rates = exchangeRatePort.getHistory(baseCurrency, targetCurrency, fromDate, toDate);
        List<ExchangeRateResponse> response = rates.stream()
                .map(ExchangeRateResponse::from)
                .toList();
        return ResponseEntity.ok(ApiResponse.success("환율 추이를 조회했습니다.", response));
    }
}
