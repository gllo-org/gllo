...
public interface CategoryJpaRepository extends JpaRepository<CategoryEntity, Long> {
    ...
    @Query("SELECT c FROM CategoryEntity c WHERE c.userId = :userId OR (c.systemCategory = true AND c.userId IS NULL) ORDER BY c.systemCategory DESC, c.name ASC")
    List<CategoryEntity> findAllByUserId(@Param("userId") UUID userId);

    boolean existsByUserIdAndName(UUID userId, String name);
    long countBySystemCategoryTrue();

    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM CategoryEntity c WHERE (c.userId = :userId OR (c.systemCategory = true AND c.userId IS NULL)) AND c.name = :name")
